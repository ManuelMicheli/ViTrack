...
export default function WeightChart({
  ...
  const fetchWeight = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const logs: WeightLog[] = await res.json();
        setRawLogs(logs);
  ...
  const handleDeleteWeight = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/weight?id=${deleteId}&user_id=${userId}`, {
        method: "DELETE",
      });
      setDeleteId(null);
      fetchWeight();
  ...
  const toGoal =
    weightGoalKg && latest
      ? Math.round(Math.abs(latest - weightGoalKg) * 10) / 10
      : null;
  ...
  // Last log indicator
  const lastLogDaysAgo = rawLogs.length > 0
    ? (() => {
        const lastDate = new Date(rawLogs[0].logged_at);
        const now = new Date();
      ...
      <div className="data-card">
            ...
            {lastLogDaysAgo !== null && (
              <span
                className={`font-mono-label text-[10px] ${
                  lastLogDaysAgo === 0 ? "text-success" : "text-text-tertiary"
                }`}
              >
                {lastLogDaysAgo === 0
                  ? t("weight.today")
                  : t("weight.daysAgo").replace("{count}", String(lastLogDaysAgo))}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Integrated weight input — filled accent CTA */}
              ...
              <AnimatePresence mode="wait">
                {showInput ? (
                  ...
                  <motion.button
                    ...
                    exit={{ opacity: 0, height: 0 }}
                    transition={springs.smooth}
                    className="w-full py-2 rounded-lg bg-[var(--color-accent-dynamic)] text-black font-mono-label hover:opacity-90 transition-all"
                  >
                    {t("weight.addWeight")}
                  </motion.button>
                )}
            ...
            {rawLogs.length > 0 && (
              <div className="mt-3 pt-3 border-t border-border">
                <button
                  ...
                  className="flex items-center justify-between w-full font-mono-label text-text-tertiary hover:text-text-secondary transition-colors"
                >
                  <span>{t("weight.history")} ({rawLogs.length})</span>
                  <motion.span
                ...
                <AnimatePresence>
                  {showHistory && (
                    ...
                    >
                      <div className="mt-2 max-h-60 overflow-y-auto space-y-0 divide-y divide-border-subtle">
                        <AnimatePresence mode="popLayout">
                          {rawLogs.slice(0, 10).map((log) => (
                            <motion.div
                              key={log.id}
